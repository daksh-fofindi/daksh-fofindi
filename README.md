# 🚀 Daksh Fofindi

### ⚡ Embedded Systems | Aerospace Developer | Hardware Engineer

I design and build **rockets, avionics systems, and embedded hardware** from scratch - including communication systems, flight computers, and launch infrastructure.

---

## 🏢 Daksh Aerospace

> Founder & CEO of DAKSH AEROSPACE
> Building **rockets, avionics, and flight-grade electronics systems**

Daksh Aerospace is focused on:

* 🚀 Rocket design & manufacturing
* ⚙️ Avionics & flight computer systems
* 📡 Communication (LoRa + custom protocols)
* 🔬 Real-world testing & iteration

🌐 Website: http://www.dakshaerospace.in/

---

## 🌐 Socials

[![Website](https://img.shields.io/badge/Website-000?style=for-the-badge\&logo=google-chrome)](http://www.dakshaerospace.in/)
[![YouTube](https://img.shields.io/badge/YouTube-red?style=for-the-badge\&logo=youtube)](https://m.youtube.com/@dakshfofindi907/videos?view=0&sort=dd&shelf_id=2)
[![Instagram](https://img.shields.io/badge/Instagram-pink?style=for-the-badge\&logo=instagram)](https://www.instagram.com/dakshfofindi/)
[![X](https://img.shields.io/badge/X-black?style=for-the-badge\&logo=twitter)](https://x.com/dakshfofindi)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge\&logo=linkedin)](https://in.linkedin.com/in/dakshfofindi)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail)](mailto:dakshfofindi@gmail.com)

---

## 🏆 Highlights

* 🚀 Building rockets, motors, and complete launch systems
* ⚙️ Designed **flight computer + receiver + ground station**
* 📡 Developed **LoRa communication with custom encryption**
* 🔧 Strong hands-on with **electronics, sensors & embedded systems**
* 🛠️ Full stack hardware: **circuit → firmware → testing**

---

## 💻 Tech Stack

### 🔧 Programming

![C](https://img.shields.io/badge/C-blue?style=for-the-badge\&logo=c)
![C++](https://img.shields.io/badge/C++-blue?style=for-the-badge\&logo=c%2B%2B)
![Python](https://img.shields.io/badge/Python-yellow?style=for-the-badge\&logo=python)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?style=for-the-badge\&logo=javascript)

### ⚙️ Embedded & Hardware

![Arduino](https://img.shields.io/badge/Arduino-green?style=for-the-badge\&logo=arduino)
![ESP32](https://img.shields.io/badge/ESP32-grey?style=for-the-badge)
![LoRa](https://img.shields.io/badge/LoRa-purple?style=for-the-badge)
![Embedded Systems](https://img.shields.io/badge/Embedded-black?style=for-the-badge)

### 🌐 Web

![HTML5](https://img.shields.io/badge/HTML5-orange?style=for-the-badge\&logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-blue?style=for-the-badge\&logo=css3)

---

## 🧪 Projects

### 🚀 Aerospace Systems

* Rocket Flight Computer
* Flight Computer Receiver
* Ground Station Software
* Automatic Rocket Launch System
* Rocket Launch Pad

### 📡 Communication Systems

* LoRa Communication System
* Custom Encryption & Packet Transmission

### 🔧 Hardware Builds

* RC Plane + Custom TX/RX
* LiPo Battery Pack
* Weighing Machine
* Studio Lightbox
* Traffic Signal System (Digital Electronics)
* 3-Band Active Equalizer (Op-Amp)

### 🚀 Rocket Engineering

* Built & tested **3 rockets (2 failures → iteration & improvement)**
* Designed motors, structures, and ignition systems

---

## 🔬 Current Work

* 🚀 Lead engineering at DAKSH AEROSPACE
* ⚙️ Next-gen flight computer
* 🛰️ Personal rocket development

---

## 🧠 Interests

* Aerospace Engineering
* Embedded Systems
* VLSI Systems
* Music
* Nuclear Physics
* Quantum Physics
* Astronomy
* Photography

---

## 🏅 Certifications & Badges

[![Responsible AI](https://img.shields.io/badge/Responsible%20AI-Google-blue?style=for-the-badge&logo=google)](https://www.skills.google/public_profiles/7b5db56b-3702-43ed-8f85-b1a960914644/badges/17270097)

[![LLM](https://img.shields.io/badge/Large%20Language%20Models-Google-orange?style=for-the-badge&logo=google)](https://www.skills.google/public_profiles/7b5db56b-3702-43ed-8f85-b1a960914644/badges/17263254)

[![Generative AI](https://img.shields.io/badge/Generative%20AI-Google-red?style=for-the-badge&logo=google)](https://www.skills.google/public_profiles/7b5db56b-3702-43ed-8f85-b1a960914644/badges/17262854)

---

## 📊 GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=dakshfofindi&show_icons=true&theme=tokyonight&count_private=true&include_all_commits=true)

![Streak](https://streak-stats.demolab.com?user=dakshfofindi&theme=tokyonight)

![Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=dakshfofindi&layout=compact&theme=tokyonight) 

---

## 🏆 GitHub Trophies

![](https://github-profile-trophy.vercel.app/?username=dakshfofindi\&theme=radical)

---

## 🧠 Developer Quote

```cpp
while(true) {
    build();
    fail();
    learn();
    rebuild();
    if(system == "working") break;
}
```

---
